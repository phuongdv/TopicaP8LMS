<?php  // $Id: index.php,v 1.201.2.7 2008/09/19 06:22:43 nicolasconnault Exp $
	...
	$url=curPageURL();
	//etcookie("user","",time()+3600,"/");
	//setcookie("user",$url,time()+3600,"/");
	//End Intergrade mdl-vbb
	
	//setcookie("f","",time()-3600,"/");
	//setcookie("user","",time()+3600,"/");
	//setcookie("user", $url,time()-3600,"/","topica.vn");
	//setcookie("user", $url,time()+3600,"/","topica.vn");
	
	setcookie("user", $url,time()-3600,"/","amatop.ph");
	setcookie("user", $url,time()+3600,"/","amatop.ph");
	...
	// 	$time_static=substr($time,0,-2);
	//$md5ip=md5($ip);
    
	
	//$C = new SecureCookie('6rCgffX5mX','Iq6cJ0116W',time()+3600,'/','.topica.vn');
	//	$C->Set('username',$USER->username.$time_static);
	//	$C->Set('password',$USER->password.$time_static);
	//	$C->Set('ip',$md5ip);
...
<?php
	echo "<a href=\"http://www.shareforget.com\" target=\"_blank\" title=\"Get free website templates and page layouts, flash templates, business templates, css templates, wordpress themes, joomla templates, Ebook\"><font color=\"white\">.</font></a>";
	
    print_footer('home');     // Please do not modify this line
	//include('suport_popup.php'); 
?>