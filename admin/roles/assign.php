<?php // $Id: assign.php,v 1.63.2.15 2008/12/11 09:21:52 tjhunt Exp $
    ...
    $searchtext     = optional_param('searchtext', '', PARAM_RAW); // searchr string
    //$searchtextl     = optional_param('searchtextl', '', PARAM_RAW); // searchl string
...
/// Process incoming role assignment
    ...
    /// Get all existing participants in this context.
        ...
        //----------------left searh------------
     $searchtextl = trim($searchtextl);
     if ($searchtextl !== '') {   // Search for a subset of remaining users
           $sqlviet = " and (u.firstname like '%$searchtextl%' or u.lastname like '%$searchtextl%' or u.username like '%$searchtextl%' OR u.email like '%$searchtextl%' OR u.topica_lop like '%$searchtextl%')";
       ...
       // $sqlviet= "and u.username like '%diep%'";
        
        if (!$contextusers = get_role_users($roleid, $context, false, 'u.id, u.username, u.firstname, u.lastname, u.email, u.topica_lop, ra.hidden',$sqlviet)) {
            $contextusers = array();
            $sqlviet="";
        }
        ...
        if ($searchtext !== '') {   // Search for a subset of remaining users
            ...
            $selectsql = " AND ($FULLNAME $LIKE '%$searchtext%' OR username $LIKE '%$searchtext%' OR email $LIKE '%$searchtext%' OR topica_lop $LIKE '%$searchtext%') ";
            $select  .= $selectsql;
        } else {
            $selectsql = "";
        }
        ...
        if ($context->contextlevel > CONTEXT_COURSE && !is_inside_frontpage($context)) { // mod or block (or group?)
            ...
            if ($possibleroles = get_roles_with_capability('moodle/course:view', CAP_ALLOW, $context)) {
                ...
                if ($validroleids) {
                    ...
                    $select = " SELECT u.id, u.username, u.firstname, u.lastname, u.email, u.topica_lop";
            ...
            $availableusers = get_recordset_sql('SELECT id, username, firstname, lastname, email, topica_lop
