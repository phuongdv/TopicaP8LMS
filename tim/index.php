...
select
	c.fullname fullname,
	FROM_UNIXTIME(c.startdate+10*3600,'%Y-%m-%d') start,
...
<li class="first"><a href="http://elearning.tvu.topica.vn/">TOPICA</a></li><li class="first"> <span class="accesshide ">/&nbsp;</span><span class="arrow sep">►</span><a href="http://elearning.tvu.topica.vn/course/view.php?id='.$cid.'">'.$coursename.'</a> </li><li class="first"> <span class="accesshide ">/&nbsp;</span><span class="arrow sep">►</span>TIM</li></ul></div>';
    ...
    $query_string = "Select u.id uid, c.fullname coursename,vt.*,r.id roleid,u.firstname,u.lastname,u.username,u.email
FROM
	mdl_user u
INNER JOIN mdl_role_assignments ra ON ra.userid = u.id
INNER JOIN mdl_context ct ON ct.id = ra.contextid
	...
	while($info = mysql_fetch_array( $data )) 
     { 
      ...
      if($info['stttuan']=='1')
      {
    	...
    	<td  width="75%"><p><font size="3"><span style="font-weight: bold;">';
		...
		if((  $roleid==13 || $roleid==211)&& checkfirstday($info['enddate'])){
...
*/      
   		...
   		File :<a href="upload/'.$info['cardgv'].'" target="_blank">PSP492 Card giảng viên</a></div>';
   		 }
   		 echo'
   		 <div style="width:120px">
   		 <img class="avatar" src="http://elearning.tvu.topica.vn/user/pix.php?file=/'.$gvcm_id.'/f1.jpg" style="float:left" >
   		 <span style="float:left;text-align:center;color:#810c15;padding:3px 3px 3px 12px;font-weight:bold">'.$gvcm.'</span>
   		 </div>
   		 <div class="bubble" style="background: #efefef; border-color: #efefef;color:#000;border-radius:10px"> 
...
*/
    ...
    <br></p>
		...
		echo '
		   <div style="width:120px">
   		 <img class="avatar" src="http://elearning.tvu.topica.vn/user/pix.php?file=/'.$gvhd_id.'/f1.jpg" style="float:left" >
   		 <span style="float:left;text-align:center;color:#810c15;padding:3px 3px 3px 12px;font-weight:bold">'.$gvhd.'</span>
   		 </div>
			<div class="bubble" style="background: #efefef; border-color: #efefef;color:#000;border-radius:10px"> 
		<p style="color:#810c15;font-weight:bold">GVHD phản hồi:'; 
		if($roleid == 14)
		{
...
\'welcome\',\'width=800,height=400,menubar=yes,status=yes\')" href=""><img src="mail.png">Gửi email tới GVHD</a></div>';
   */
		...
		else if($info['emailphanhoi']!=1 && $roleid==14 && $info['phanhoigvhd']!='')
		{
		echo'<a name="today" class="fancybox"  href="sendmail_gvhd.php?w='.$info['id'].'&c='.$cid.'"><img src="mail.png">Gửi email phản hồi tới GVCM</a>';
		}
		
					else if($info['emailphanhoi']==1 && $roleid==14)
		{
		echo '<div style="margin-bottom:0px"><br><br><hr><br>Đã gửi E-mail cho GVCM </div>';
		}
