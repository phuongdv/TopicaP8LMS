...
// Get SMS INFO ID
function getSMS_INFO_ID($userid,$date){
	global $CFG;
	$mysqli = new mysqli($CFG->dbhost,$CFG->dbuser,$CFG->dbpass);
	...
	$sqlSMSINFO="select sms_info_id from sms_info where usercreat='$userid' and reg_date='$date'";
...
<table cellpadding="5" cellspacing="0" width="600px" border="0" class="girdtable" align="center">
    ...
    <tr>
        ...
        <br>
        Số ký tự : <input readonly type="text" name="CharsTyped" size="8" value="" maxlength="160" max="160" > 
        <br>
...
<?
	...
	$mysqli->close();
		...
		//print_r($sql);
			...
			//GET SMS INFO ID
			//echo "da den";
			$sms_info_id=getSMS_INFO_ID($userid,$date);
			//print_r($sms_info_id);
			...
			if (mysqli_num_rows($qcount_send) > 0){
				while($cc = $qcount_send->fetch_assoc()) 
					{
						$useridin=$cc["id"];
						$topica_dienthoaiin=$cc["topica_dienthoai"];
						$sqlsmssend = "
