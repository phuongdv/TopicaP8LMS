...
while($arr_de=mysql_fetch_assoc($result)){
    ...
    wwwcopy("http://elearning.tvu.topica.vn/mod/bt30/update_file_html.php?id=$de_id",$file_quiz);
			// end update content
	...
	//echo count($questids_array).'<br>';
//	for($i=0;$i<count($questids_array);$i++){
	//	echo $questids_array[$i].'<br>';
		...
		$result=mysql_query($sql_question);
		$arr_question_info=mysql_fetch_assoc($result);

		$content_quest=$arr_question_info["questiontext"];
	    echo $content_quest.'<br>';*/
//	} // end for*/
