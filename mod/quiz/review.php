...
/// Print all the questions
    ...
    foreach ($pagequestions as $i) {
        ...
        // Print the question
       // $qerr = '';
       //Thêm nút báo lỗi Huypt
        if ($err == trim($states[$i]->question)){ echo '<div style="position: relative; background-color:#FF0000"><a name="err"></a>'; }
        else { echo '<div style="position: relative">'; }
     // echo ' <div style="position: absolute; top: 60px;left:10px;border: 1px solid;"><a target="_blank" href="rp_que.php?a='.$states[$i]->attempt.'&q='.$states[$i]->question.'&c='.$course->id.'&ss='.$states[$i]->id.'&n='.$states[$i]->options->question->name.'"> Góp ý </a></div>';
        echo ' <div style="position: absolute; top: 60px;left:10px;border: 1px solid;"><a target="_blank" href="http://ccms.topica.vn/mycc.php?attempt='.$_GET['attempt'].'&question='.$states[$i]->question.'"> Góp ý </a></div>';
