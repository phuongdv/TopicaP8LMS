...
class quiz_report extends quiz_default_report {
    ...
    function display($quiz, $cm, $course) {
		...
		//danglx: Them chuc nang tim kiem theo ten 6-11-2013
		echo '
		<form method="post" action="">
		<table border="0">
		<tr>
			<td>Nhập họ và tên đệm hoặc tên
			</td>
			<td>
		...
		if(isset($_POST["btnsearch"])){
			...
			if ( $keyword =='')
				...
				{
					$whereclause = " AND (u.firstname LIKE '%$keyword%' or u.lastname LIKE '%$keyword%')  ";
				}
		
		
		//End chuc nang tim kiem theo ten
		...
		where tuaq.quiz=$quizid and tuaq.deleted =0 ".$whereclause." order by uid , start_time";	
		...
		while($dd = $ad->fetch_assoc()) 
		{
		  if($dd['uid']==$uid)
			...
			echo '
			  ...
			  //echo '<td align="center">'.$dd['diem'].'</td>';
				  
			  echo '<td align="center">';
			  //danglx thêm nếu là disconnected thì hiển thị điểm 0
			  if ( $dd['status']<> 'disconnected')
			  {
			  	echo $dd['diem'];
			  }
			  else echo '0.00';
			 ...
			 if($candelete)
				...
				//danglx modified 21-03-2014
				$sql_hv_chon = "select answer from vietth_q169_answer where attempt = ".$dd['tuaqid']." order by id desc limit 0,1";
