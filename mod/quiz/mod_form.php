...
//-------------------------------------------------------------------------------
		...
		//tinhth: add loai quiz ap dung
        $quiz_apply_options = array(moodle_quiz =>moodle_quiz,
							 bt72_quiz =>bt72_quiz,
							 bt30_quiz =>bt30_quiz);
        $mform->addElement('select', 'quiz_type_apply','Loại quiz áp dụng', $quiz_apply_options);
        $mform->setDefault('quiz_type_apply',(int)$ass->quiz_type_apply);
		//end tinhth: add gioi han disconnect
		
		//tinhth: add gioi han disconnect
        $dis_connect_options = array(3 =>3,
							 4 =>4,
							 5 =>5,
							 6 =>6);
        $mform->addElement('select', 'disconnect','Số lần cho phép Mất kết nối', $dis_connect_options);
        $mform->setDefault('disconnect',(int)$ass->disconnect);
		//end tinhth: add gioi han disconnect
        ...
        $timedelayoptions[300] = get_string('numminutes', '', 5);
        $timedelayoptions[600] = get_string('numminutes', '', 10);
        for($i=2; $i<=23; $i++) {
             $seconds  = $i*3600;
