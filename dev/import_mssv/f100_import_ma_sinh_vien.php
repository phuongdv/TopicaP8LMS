...
<title>f100 - Cập nhật mã sinh viên</title>
<style type="text/css">
<!--
.style2 {color: #810C15}
-->
...
<body>
<div align="center"><span class="style2"><h1>F100 - CẬP NHẬT MÃ SINH VIÊN</h1></span></div>

    	<form enctype="multipart/form-data" action="" method="post">
              <table width="70%" align="center" >
              <tr><td colspan="4"><a href="ds_msv.xls">Download file mẫu</a> | <a href="huongdan_import_msv.doc">Hướng dẫn import mã sinh viên</a></td></tr>
              <tr>
              ...
              <td><strong>Bước 4</strong>: Cập nhật mã sinh viên</td>
              <td><input type="submit" name="update"  value="4. Cập nhật mã sinh viên" /></td>
              </tr></form>
...
if ($_POST['upfile'])
{
	...
	for ($i=4;$i<=$rowsnum;$i++) // Duyệt từng hàng, bắt đầu lấy dữ liệu từ hàng 4
	{
		...
		/* Lưu dữ liệu vào DB
		/*---------------------------------------------*/
		$sql = "insert into tmp_import_ma_sinh_vien(username, ma_sinh_vien, ho_ten, ghi_chu) values(";
		$sql .= "'".$data->val($i,2)."'";
		$sql .= ",'".$data->val($i,3)."'";
		$sql .= ",'".$data->val($i,4)."'";
		$sql .= ",'".$data->val($i,5)."');";
	...
	if ($_POST['update'])
	{
		...
		while ($row = mysql_fetch_array($result))
		{
				...
				$sql_update_user = "update mdl_user set topica_msv = '".$row['ma_sinh_vien']."'where username = '".$row['username']."'";
		...
		$tong_so_sinh_vien = mysql_num_rows($result); // Tong so ban ghi
			...
			echo "</table>";
		//load du lieu len form goi trang ds_ma_sinh_vien
			include("ds_ma_sinh_vien.php");
	}	
