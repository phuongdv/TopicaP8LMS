...
<title>F500 - CẬP NHẬT NHÓM CHO SINH VIÊN</title>
<style type="text/css">
<!--
.style2 {color: #810C15}
-->
...
<body>
<div align="center"><span class="style2"><h1>F500 - CẬP NHẬT THÔNG TIN NHÓM CHO SINH VIÊN</h1></span></div>

    	<form enctype="multipart/form-data" action="" method="post">
              <table width="70%" align="center" >
              <tr><td colspan="4"><a href="ds_nhom.xls">Download file mẫu</a> | <a href="huongdan_import_nhom.doc">Hướng dẫn import nhóm</a></td></tr>
              <tr>
              ...
              <td><strong>Bước 4</strong>: Cập nhật nhóm</td>
              <td><input type="submit" name="update"  value="4. Cập nhật nhóm" /></td>
              </tr></form>
...
if ($_POST['upfile'])
{
	//1. xoa trang du lieu trong ban tam tmp_import_ma_sinh_vien truoc khi import dot moi
	$sql_delete = "delete from tmp_import_ma_sinh_vien ";
	...
	for ($i=7;$i<=$rowsnum;$i++) // Duyệt từng hàng, bắt đầu lấy dữ liệu từ hàng 4
	{
		...
		/* Lưu dữ liệu vào DB
		/*---------------------------------------------*/
		$sql = "insert into tmp_import_ma_sinh_vien(username, ho_ten, chucvutronglop, nhom, chucvutrongnhom, ghi_chu) values(";
		$sql .= "'".$data->val($i,2)."'";
		$sql .= ",'".$data->val($i,3)."'";
		$sql .= ",'".$data->val($i,4)."'";
		$sql .= ",'".$data->val($i,5)."'";
	...
	if ($_POST['update'])
	{
		$sql_kiem_tra = " SELECT * FROM tmp_import_ma_sinh_vien where ket_qua_kiem_tra_yn = 'Y'";
		...
		while ($row = mysql_fetch_array($result))
		{
				...
				$sql_update_user = "update mdl_user set topica_chucvutronglop = '".$row['chucvutronglop']."', topica_nhom = '".$row['nhom']."',topica_chucvutrongnhom ='".$row['chucvutrongnhom']."' where username = '".$row['username']."'";
	...
	while ($row = mysql_fetch_array($result))
		{
			...
			if(mysql_num_rows($ket_qua)>0)
			{
				...
				$sql_update = "update tmp_import_ma_sinh_vien set ket_qua_kiem_tra_yn = 'Y' where id=".$row["id"];
				$ket_qua = mysql_query($sql_update);
			}
			else
			{
				//neu khong ton tai sinh vien thi update N va comment
				$sql_update = "update tmp_import_ma_sinh_vien set ket_qua_kiem_tra_yn = 'N', ghi_chu_kiem_tra ='username không tồn tại'
																where id=".$row["id"];
				$ket_qua = mysql_query($sql_update);
			}
		}
