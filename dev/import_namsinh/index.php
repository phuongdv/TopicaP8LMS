...
<title>F400 - CẬP NHẬT NGÀY SINH CHO SINH VIÊN</title>
<style type="text/css">
<!--
.style2 {color: #810C15}
-->
...
<body>
<div align="center"><span class="style2"><h1>F400 - CẬP NHẬT NGÀY SINH CHO SINH VIÊN</h1></span></div>

    	<form enctype="multipart/form-data" action="" method="post">
              <table width="70%" align="center" >
              <tr><td colspan="4"><a href="ds_namsinh.xls">Download file mẫu</a> | <a href="huongdan_import_namsinh.doc">Hướng dẫn import ngày sinh</a></td></tr>
              <tr>
              ...
              <td><strong>Bước 4</strong>: Cập nhật ngày sinh</td>
              <td><input type="submit" name="update"  value="4. Cập nhật ngày sinh" /></td>
              </tr></form>
...
if ($_POST['upfile'])
{
	...
	for ($i=4;$i<=$rowsnum;$i++) // Duyệt từng hàng, bắt đầu lấy dữ liệu từ hàng 4
	{
		...
		/* Lưu dữ liệu vào DB
		/*---------------------------------------------*/
		$sql = "insert into tmp_import_ma_sinh_vien(username, ho_ten, nam_sinh, ghi_chu) values(";
		$sql .= "'".$data->val($i,2)."'";
		$sql .= ",'".$data->val($i,3)."'";
		$sql .= ",'".$data->val($i,4)."'";
		$sql .= ",'".$data->val($i,5)."');";
	...
	if ($_POST['update'])
	{
		...
		while ($row = mysql_fetch_array($result))
		{
				...
				$sql_update_user = "update mdl_user set topica_namsinh = '".$row['nam_sinh']."'where username = '".$row['username']."'";
