...
class block_admin extends block_list {
    ...
    /// View reportlipe
        if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a target="_blank" href="/lipe">'.get_string('reportlipe').'</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
        /// View reportlipe 2014 v2
       // if ($USER->username == 'danglx' || $USER->username == 'anhnt2' || $USER->username == 'trungtt' || $USER->username == 'nhungptt.qlht' || $USER->username == 'nhungptt.pm' || $USER->username == 'nhungptt' || $USER->username == 'vantt2' || $USER->username == 'tinhth' || $USER->username == 'hatvv')
       // {
	    ...
	    {	
	        if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
	            $xx="demo";
	            $b64=base64_encode($xx);
	            $this->content->items[]='<a target="_blank" href="/lipe2014/interaction_base_web.php">Báo cáo lipe 2014 (Dành cho các course có ngày bắt đầu từ 27/04/2014)</a>';
	            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
	        }
        }
        else
        {
        	if( $role==1 || $role==11 || $role==211 )
        	{
				if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
		            $xx="demo";
		            $b64=base64_encode($xx);
		            $this->content->items[]='<a target="_blank" href="/lipe2014/interaction_base_web.php">Báo cáo lipe v2014 (Dành cho các course có ngày bắt đầu từ 27/04/2014)</a>';
		            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
		        }
	        }
		}
     /// End View reportlipe 2014 v2   

    /// View quiz reports - hieupd
        if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $this->content->items[]='<a href="'.$CFG->wwwroot.'/course/quizreport.php?id='.$this->instance->pageid.'">Bao cao tinh hinh lam quiz</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
	  /// View lmsreport
        if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a target="_blank" href="/lmsreport/weekreport/">TVU-BC001</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
		if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a target="_blank" href="/lmsreport/weekreport/bc002.php">TVU-BC002</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
		if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a target="_blank" href="/lmsreport/weekreport/bcgvcm.php">TVU-BCGVCM</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
		if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a target="_blank" href="/lmsreport/weekreport/bcgvdn.php">TVU-BCGVDN</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/stats.gif" class="icon" alt="" />';
        }
    /// View course danhsachlop2 - hieupd
        ...
        if ($course->id !== SITEID and has_capability('moodle/site:viewreports', $context) and $USER->topica_staff =='1') { // basic capability for listing of reports
            $this->content->items[]='<a href="'.$CFG->wwwroot.'/user/danhsachlop.php?limit=30">'.get_string('participants').'</a>';
	...
	// err quest by Huypt
        if ($course->id !== SITEID and has_capability('moodle/course:managefiles', $context)) {
            $this->content->items[]='<a href="'.$CFG->wwwroot.'/mod/quiz/rp_view.php?c='.$this->instance->pageid.'">Question error</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/questions.gif" class="icon" alt="" />';
        }
        if ($course->id !== SITEID and has_capability('moodle/course:managefiles', $context)) {
            $this->content->items[]='<a href="'.$CFG->wwwroot.'/question/edit2.php?courseid='.$this->instance->pageid.'">Question error FEEDBACK</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/questions.gif" class="icon" alt="" />';
        }
    /// Manage files
        if ($course->id !== SITEID and has_capability('moodle/course:managefiles', $context)) {
	...
	// danglx: Link to the Import ma sinh vien
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_mssv/f100_import_ma_sinh_vien.php" target="_blank">Import mã sinh viên</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import ma sinh vien
	
	// danglx: Link to the Import so dien thoai
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_dienthoai/f200_import_sinh_vien.php" target="_blank">Import số điện thoại</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import so dien thoai
	
	// danglx: Link to the Import nam sinh
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_namsinh" target="_blank">Import năm sinh cho SV</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import nam sinh
	
	// danglx: Link to the Import nhom cho SV
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_nhom" target="_blank">Import nhóm cho SV</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import nhom cho SV
	
	// danglx: Link to the Import email
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_email" target="_blank">Import email cá nhân</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import email
	
	// danglx: Link to the Import offline
		if ($course->id !== SITEID and has_capability('moodle/site:import', $context)) { // basic capability for listing of reports
            $xx="demo";
            $b64=base64_encode($xx);
            $this->content->items[]='<a href="/dev/import_offline" target="_blank">Import buổi offline</a>';
            $this->content->icons[]='<img src="'.$CFG->pixpath.'/i/restore.gif" class="icon" alt="" />';
        }
		
	// end danglx: Link to the Import offline
        return $this->content;
    }
