...
define('COURSE_MAX_COURSES_PER_DROPDOWN',2000); //  max courses in log dropdown before switching to optional
...
if (!defined('FRONTPAGECOURSELIMIT')) {
    define('FRONTPAGECOURSELIMIT',    2000);     // maximum number of courses displayed on the frontpage
}
...
function print_log($course, $user=0, $date=0, $order="l.time ASC", $page=0, $perpage=100,
    ...
    if ($course->id == SITEID) {
        $courses[0] = '';
        if ($ccc = get_courses('all', 'c.DESC', 'c.id,c.shortname')) {
            foreach ($ccc as $cc) {
...
function print_mnet_log($hostid, $course, $user=0, $date=0, $order="l.time ASC", $page=0, $perpage=100,
    ...
    if ($course->id == SITEID) {
        $courses[0] = '';
        if ($ccc = get_courses('all', 'c.DESC', 'c.id,c.shortname,c.visible')) {
            foreach ($ccc as $cc) {
                $courses[$cc->id] = $cc->shortname;
            }
        }
...
function print_log_csv($course, $user, $date, $order='l.time DESC', $modname,
    ...
    if ($course->id == SITEID) {
        $courses[0] = '';
        if ($ccc = get_courses('all', 'c.id DESC', 'c.id,c.shortname')) {
            foreach ($ccc as $cc) {
                $courses[$cc->id] = $cc->shortname;
            }
        }
...
/// This function is only used by print_whole_category_list() above
    ...
    if ($showcourses and $coursecount) {
        ...
        if ($depth) {
          ...
          echo '<td width="20" rowspan="'.$rows.'" valign="top">';
           print_spacer(10, $indent);
          echo '</td>';
        }

        echo '<td valign="top" class="category image" width="20">'.$catimage.'</td>';
      ...
      //does the depth exceed maxcategorydepth
        ...
        if ($courses && ($limit || $CFG->maxcategorydepth == 0)) {
            foreach ($courses as $course) {
                ...
                echo '<a '.$linkcss.' href="'.$CFG->wwwroot.'/courses/view.php?id='.$course->id.'">'. format_string($course->fullname).'</a>';
        ...
        echo '<td valign="top" class="category name" ><div align="left">';
        echo '<a '.$catlinkcss.' href="'.$CFG->wwwroot.'/course/category.php?id='.$category->id.'">'. format_string($category->name).'</a>';
        echo '</div></td>';
...
/// begin courses list combo box by hieupd 20091027_1605
function print_courses_list($category) {
    ...
    if (!is_object($category) && $category==0) {
        ...
        if (is_array($categories) && count($categories) == 1) {
            ...
            $courses    = get_my_courses($USER->id, 'visible DESC,id DESC', array('summary'));
        } else {
            $courses    = get_my_courses($USER->id, 'visible DESC,id DESC', array('summary'));
        }
        unset($categories);
    ...
    if ($courses) {
        ...
        echo '<div align="right"><select onchange="document.location.href = this.value;">';
		echo '<option>C&#225;c l&#7899;p h&#7885;c c&#7911;a t&#244;i</option>';
        foreach ($courses as $course) {
            if ($course->visible == 1
                || has_capability('moodle/course:viewhiddencourses',$course->context)) {
                echo '<option value="'.$CFG->wwwroot.'/course/view.php?id='.$course->id.'">';
        ...
        echo '<form id="'.$id.'" action="'.$CFG->wwwroot.'/course/search.php" method="get">';
        echo '<fieldset class="coursesearchbox invisiblefieldset">';
        echo '<label for="shortsearchbox">'.$strsearchcourses.': </label>';
        echo '<input type="text" id="shortsearchbox" size="12" name="search" alt="'.s($strsearchcourses).'" value="'.s($value, true).'" />';
        echo '<input type="submit" value="'.get_string('go').'" />';
        echo '</fieldset></form><br />';
    }
}
/// end courses list combo box by hieupd 20091027_1605
...
// Edit by vietth to hidden teacher on front page ------------------
        if (!empty($namesarray)) {
