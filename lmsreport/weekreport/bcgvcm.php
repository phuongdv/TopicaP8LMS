...
AND tblreply.userid=$c_id
 ...
 function count_nx($c_id,$start_time=0,$end_time=0)
 {
 global $DB;
 
 	$sql_time='';
  if($start_time != 0)
   $sql_time = " and DATE(FROM_UNIXTIME($start_time)) < startdate ";
  if($end_time !=0)
   $sql_time .=" and DATE(FROM_UNIXTIME($end_time)) > startdate ";
 
 
 
 $sql="select count(*) count from vietth_tam where nhanxet !='' and course=$c_id $sql_time";
 $result = $DB->fetch_assoc($sql);
  return $result[0]['count'];
 }
...
function count_xem($c_id,$u_id,$start_time=0,$end_time=0)
{
	global $DB;
	$sql_time='';
  if($start_time != 0)
   $sql_time = " and time > $start_time ";
  if($end_time !=0)
   $sql_time .=" and time < $end_time ";
   
	$sql ="select count(*) count from mdl_log where  action='view' and
       course = $c_id and userid = $u_id
	   $sql_time
       ";
	   $result = $DB->fetch_assoc($sql);
	   return $result[0]['count'];
...
<title>Báo cáo hoạt động giảng viên chuyên môn</title>
...
<iframe width="1000px" scrolling="no" height="90px" frameborder="0" border="0" framespacing="0" src="/lmsreport/weekreport/head.html"></iframe>
		...
		<p>From date:  <input name="start" class="easyui-datebox" value="<?php  echo $_POST['start']; ?>"></input>   To date  :  <input name="end" class="easyui-datebox" value="<?php echo $_POST['end'];?>"></input> &nbsp; &nbsp; &nbsp; <label ><input  type="checkbox" name="excel" value="excel"> Excel </label><input style="float:right;margin-right:10px;background:#fff;heigth:30px;border:solid 1px #810c15" type="submit" value=" Xem "></p>
        </div>
		</form>
		<div style="margin-top:20px"></div>
	<?php
...
<table border="1" style="border-collapse:collapse;font-size:10pt" id="data_g" width="1000" cellpadding="3" cellspacing="0">
  ...
  <tr>
	...
	<th rowspan="2" width="227"><div align="center">Lecturer</div></th>
    ...
    <th rowspan="2" width="105"><div align="center">Start date</div></th>
     ...
     <th rowspan="2" width="105"><div align="center">week</div></th>
	  <th colspan="3"><div align="center">Login Class</div></th>
    <th colspan="2"><div align="center">Reply H2472</div></th>
    <th colspan="2"><div align="center">Forum</div></th>
  </tr>
  <tr>
      <th  width="93"><div align="center">Times</div></th>
	    <th  width="93"><div align="center">Số yêu cầu / tuần</div></th>
		  <th  width="93"><div align="center">Tổng số đến thời điểm báo cáo</div></th>
      <th  width="93"><div align="center">Số Câu được assign</div></th>
     <th " width="93"><div align="center">Số câu trả lời quá hạn</div></th>
    <th  width="93"><div align="center">Số nhận xét</div></th>
   
    <th ><div align="center">Số bài post</div></th>
  </tr>
  ...
  <tr <?php echo $stt%2 ? '': 'style="background:#eee"'?>>
    ...
    <td align="right" style="padding-right:15px"><?php echo date('d-m-Y',$report['enrolenddate']);?></td>
	    <td align="right" style="padding-right:15px"><?php echo tuan_hien_tai ($report['cid'],$end) ;?></td>
		 <td align="right" style="padding-right:15px"> <?php
	 echo count_xem($report['cid'],$report['uid'],$start,$end);
	 ?></td>
	 <td align="right" style="padding-right:15px"> 
	 
	 <?php /* Cần sửa sau (ThangDM's Note)*/
	 $tb= get_tb($report['shortname']);
	 echo $tb['yc_dang_nhap_tuan_gvcm'];
	 ...
	 <td align="right" style="padding-right:15px">
	 <?php
	 echo count_xem($report['cid'],$report['uid']);
	 ?>
    ...
    <td align="right" style="padding-right:15px"><?php echo count_h2472_qua_han($report['uid'],$report['cid'],$start,$end);?></td>
	 <td align="right" style="padding-right:15px">
	<?php
	echo count_nx($report['cid'],$start,$end);
	?>
	</td>
    <td align="right" style="padding-right:15px"><?php 
	$forumid = get_forum_id($report['cid']);
	if($forumid!='')
	{
	$forum_post = count_post_forum($start,$end,$forumid,"'".$report['username']."'");
	echo $forum_post ;
    }
	else
	{
