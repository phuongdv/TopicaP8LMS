<?php
  ...
  class DB
  {
    ...
    function DB($base, $server, $user, $pass)
    {
      ...
      mysql_connect($server, $user, $pass) or die('Khong the ket noi den Mysql Server.');
      mysql_select_db($base)               or die('Khong the ket noi den Database.');
    }
    ...
    function execute($query, $debug = -1)
    {
	  ...
	  /** Insert.
      * @param $data The array bin data,$table : table use
      * @return affect row.
      */
    ...
    function fetch_assoc($query, $debug = -1)
    {
      $this->nbQueries++;
      $this->lastResult = mysql_query($query) or $this->debugAndDie($query);
      $this->debug($debug, $query, $this->lastResult);
      while( $data = mysql_fetch_assoc($this->lastResult))
      {
      	$result[]= $data;
      }
      return $result;
    ...
    /** Get the count of rows in a table, with a condition.
      ...
      */
    function countOf($table, $where=' 1')
    {
