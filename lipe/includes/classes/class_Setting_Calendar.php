	...
	*/
	...
	function countPostInWeekVBB($username,$start,$end,$active_id) {
		...
		*/
		...
		$sql="select count(vp.postid) from vbb_post vp,vbb_thread vt where vt.forumid = $active_id 
					and vt.threadid = vp.threadid 
					and vp.username= '".$username."' and (vp.dateline > UNIX_TIMESTAMP('$start_date') and vp.dateline < UNIX_TIMESTAMP('$end_date'))";
	...
	# Time : 10/01/2014
	/*
	function getRelationCalendarAndLipesettingVBB_dang($username,$id,$c_id) {

		global $dbconn;
		
		$sql_dang ="SELECT count(1) count FROM huy_setting_calendar WHERE c_id=$c_id AND week_name <> 'Tổng'";
		$res_dang = $dbconn->GetAll($sql_dang);
		if(is_array($res_dang)) {
			$count_week = $res[0]["count"];
			$end_week = 'Tuần '.$count_week;
		}
		
		$sql2 = "select c.id,c.week_name,c.start_date,c.end_date,l.lipe_type,l.active_id from huy_setting_calendar c, huy_setting_lipe l where c.id = l.calendar_id and c.c_id= $c_id and c.id= $id and l.lipe_type='V'";
		$res = $dbconn->GetAll($sql2);
		if(is_array($res)) {
			
			if ($res[0]["week_name"] == $end_week || $res[0]["week_name"] == 'Tổng')
				{
					$start_date_1 = date('Y-m-d',$res[0]["end_date"]);
					$start_date = strtotime($start_date_1) + 86399;				
					$variable = $this->countPostInWeekVBB($username, date('Y-m-d',$res[0]["start_date"]), date('Y-m-d H:i:s',$start_date), $res[0]["active_id"]);
				}
			else
				{
			//$variable = ($res[0]["lipe_type"]=='V')? $this->countPostInWeekVBB($username, date('Y-m-d',$res[0]["start_date"]), date('Y-m-d',$res[0]["end_date"]), $res[0]["active_id"]) : 0;
	...
	function countPostInWeekVBB_dang($username,$start,$end,$active_id) {
...
function GradesInWeektest_dang($u_id,$arr_active_id) {
	...
	function CountPractice ($u_id,$c_id,$start_date,$end_date) {
		...
		AND q.timefinish <= q.timemodified
		";
		...
		AND q.timefinish <= $end_date
		";
		// danglx them AND q.timefinish <= q.timemodified dòng trên
		$res = $dbconn->GetAll($sql5);
...
";
	...
	function ConvertDateGMT($a)
 	...
 	function getCalendarInLipeCN($c_id){
		...
		//danglx modified 17/1/2014
		$sql="select id,week_name,start_date,end_date, (SELECT COUNT(1) FROM huy_setting_calendar WHERE c_id=$c_id) count from huy_setting_calendar where c_id=$c_id";
		$res = $dbconn->GetAll($sql);
	...
	* @author: Danglx
	* Date: 19-02-2014
	* @name: Kiem tra SV lam bai tap hay chua BT30, BT72
	* @return: 1 neu da lam.
	* @return: 0 neu chua lam
	*/
	function checkDoQuiz2($user_id,$quiz_id){
		global $dbconn;
		$sqlEx="select count(*) 
				from vietth_q169_attempts
				where
 				quiz = $quiz_id
				and userid=$user_id";
