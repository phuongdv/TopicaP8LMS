...
function SaveCc($c_id,$u_id,$cc,$sum_grade=0,$total_grade_scale=1)
   {
   global $dbconn;
   //$cc=(float)$cc;
   $kt=ceil(($sum_grade*100)/$total_grade_scale);
   
/*    if($c_id!='' && $u_id !='' && $cc!='')
    { */
	 $sql_check = "select count(*) count from diemcc where cid=$c_id and uid = $u_id";
	 $res_check = $dbconn->GetAll($sql_check);
	 if($res_check[0]['count']==0)
	 {
	 $sql="insert diemcc(diemcc,cid,uid,time_modified) values ('$cc','$c_id','$u_id','".date(time())."' )";
     $dbconn->Execute($sql);
	 }
	 else
	 {
	 $sql="update diemcc set diemcc=$cc,diemkt=$kt,time_modified=".date(time())."  where cid=$c_id and uid=$u_id ";
	  $dbconn->Execute($sql);
	 //$dbconn->GetAll($sql);
	 }
	//}
...
function getCCTemp($cid,$uid)
    {
	global $dbconn;
	 $sql = "select diemcc from diemcc where cid = ".$cid." and uid = ".$uid;
	 $res = $dbconn->GetAll($sql);
     return $res[0]['diemcc'];
	 //return $res;
	}
function getTime_modified($cid,$uid)
    {
	global $dbconn;
	 $sql = "select FROM_UNIXTIME(time_modified) time_modified from diemcc where cid = ".$cid." and uid = ".$uid;
	 $res = $dbconn->GetAll($sql);
     return $res[0]['time_modified'];
	}
}
?>