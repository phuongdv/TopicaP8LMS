...
class Users extends Public_Controller
{
	...
	public function register()
	{
		...
		if ($this->input->post())
		{
			if(!validateCaptcha($_POST['captcha_code'])){
				$this->session->set_flashmessage('error','Mã kiểm tra không chính xác !');
	...
	public function reset_pass($code = null)
	{
		...
		if($this->input->post()){
			if(!validateCaptcha($_POST['captcha_code'])){
				...
				if ($user_meta)
				{
					$new_password = $this->ion_auth->forgotten_password($user_meta->id);
	...
	{
		if($id==1&&$this->current_user->id!=1) redirect('/');
		...
		if ($this->form_validation->run())
		{
			...
			if (isset($secure_post['password']))
			{
				//CaoPV:Không cho đổi mật khẩu ở đây
	...
	function change_password(){
		...
		$this->validation_rules = array(
			...
			array(
				'field' => 'password',
				'label' => 'Mật khẩu mới',
				'rules' => 'required|xss_clean|trim|min_length[6]|max_length[50]'
			),
			array(
		...
		if($this->input->post()){
			...
			{
				try {
					$check=$this->ion_auth->change_password($user->id,trim($this->input->post('current_password')),trim($this->input->post('password')));
					if($check){
						$this->session->set_flashdata('success','Đổi mật khẩu thành công !');
						redirect('users');
					}else{
						$this->session->set_flashmessage('error',"Mật khẩu hiện tại không chính xác ! ".$this->ion_auth->errors());
						//$this->session->set_flashmessage('error', $this->ion_auth->errors());
					}
				} catch (Exception $e) {
					$this->session->set_flashmessage('error',$e->getMessage());
				}
			}
			else
			{
