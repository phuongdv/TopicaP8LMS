...
class Page_m extends MY_Model
{
	...
	{
		$where_params=array($uri,$is_request,$simple_return);
		if($this->_cache_on){
		...
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$where_params,$page);
		}
	...
	{
		$where_params=array($id,$get_data);
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where_params);
			if($result)
				return $result;
	...
	{
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__);
			if($result)
				return $result;
		}
		$result=$this->db
			...
			->row();
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,array(),$result);
		}
		return $result;
	}
	...
	public function get_page_tree()
	{
		...
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,array(),$page_array);
		}
	...
	{
		$where_params=array($stream_entry_id,$stream_slug);
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where_params);
			if($result)
				return $result;
		}
		$result=$this->streams->entries->get_entry($stream_entry_id, $stream_slug, 'pages');
		if($this->_cache_on){
	...
	{
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$uri);
			if($result)
				return $result;
		}
		$result=$this->db
			...
			->row();
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$uri,$result);
		}
		return $result;
	}
	...
	{
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$id);
			if($result)
				return $result;
		}
		$result=$this->db
			...
			->result();
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$id,$result);
		}
		return $result;
	}
	...
	public function get_descendant_ids($id, $id_array = array())
	{
		...
		$where_params=array($id,$id_array);
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where_params);
			if($result)
				return $result;
		...
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$where_params,$id_array);
		}
