...
class Cache_Model
{
	 ...
	 * @return bool
	 */
	public function _read($property, $method, $arguments = array(), $expires = NULL){
		...
		$cache_file = $property.DIRECTORY_SEPARATOR.$method.DIRECTORY_SEPARATOR.do_hash($_SERVER['SERVER_NAME'].$method.serialize($arguments), 'sha1');
	 ...
	 * @return mixed
	 */
	public function _write($property, $method, $arguments = array(),$object,$expires = NULL){
		...
		$this->write($object,$cache_file, $expires);
	 ...
	 * @param string $method
	 */
	function _delete($property,$method=''){
		if($method=='insert'||$method=='insert_many'){
			//no delete get/get_by
			$dirs=array('get_many_by','get_many_where','count_by');
			foreach($dirs as $dir){
				$cache_dir = $property.DIRECTORY_SEPARATOR.$dir;
				$this->delete_all($cache_dir);
			}
		}else{
			$this->delete_all($property);
		}
	}
