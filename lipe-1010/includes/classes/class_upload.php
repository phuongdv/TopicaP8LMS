...
class Upload_Files{
    ...
    var $banned_array;
    var $ext_array = array(".jpg",".jpeg",".gif",".png",".bmp"); 
	var $error_text;
	function validate_extension() {
		...
		//SECTION #2
		if (!$file_name) {
			$this->error_text = "File name is none !";
			return false;
		} else {
			if (!$ext_array) {
				return true;
				...
				if ($valid_extension) {
					...
					$this->error_text = "Extension file is not allow !";
					return false;
				}
			}
		}
...
//echo "tmp=$temp_file_name  <br>";
		$max_file_size = $this->max_file_size;
		if ($temp_file_name!="") {
				$size = filesize($temp_file_name);
				if ($size > $max_file_size) {
					$this->error_text = "Size of file is too big !";
					return false;                                                        
				} else {
					return true;
				}
	...
	function existing_file() {
		...
		if ($upload_dir == "ERROR") {
			...
			if (file_exists($file)) {
				...
				$this->error_text = "This file is exist !";
				return false;
			}
		}    
	}
	...
	function upload_file_no_validation() {
		...
		//SECTION #2
		if (($upload_dir == "ERROR")) {
			writeLog("ERROR", "Upload1", $_SERVER['PHP_SELF'], $_SERVER['PHP_SELF'], "Upload dir is not found:".$this->upload_dir."\r\n");
			return false;
		} else {
...
/*					$log = $upload_log_dir.$y."_".$m."_".$d.".txt";
					...
					writeLog("ERROR", "Upload2", $_SERVER['PHP_SELF'], $_SERVER['PHP_SELF'], "Can not upload file:".$this->file_name."\r\n");
					return false;    
				}
			} else {
				writeLog("ERROR", "Upload3", $_SERVER['PHP_SELF'], $_SERVER['PHP_SELF'], "Can not find file:".$this->file_name."\r\n");
				return false;
			}
		}
	}
	function upload_file_with_validation() {
		...
		elseif ((((!$valid_user) OR (!$valid_size) OR (!$valid_ext) OR ($existing_file)))) {
			writeLog("ERROR", "Upload4", $_SERVER['PHP_SELF'], $_SERVER['PHP_SELF'], "File is too large or invalid extension:".$this->file_name."\r\n");
			return false;
		} else {
			if (is_uploaded_file($temp_file_name)) {
				if (move_uploaded_file($temp_file_name,$upload_dir . $file_name)) {
				...
				writeLog("ERROR", "Upload3", $_SERVER['PHP_SELF'], $_SERVER['PHP_SELF'], "Can not find file:".$this->temp_file_name."\r\n");
				return false;
			}
		}
	} 
