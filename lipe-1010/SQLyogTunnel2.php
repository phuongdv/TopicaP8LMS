...
/*
    ...
    Sabyasachi Ruj <ruj.sabya@gmail.com>

*/
define("DB_HOST", "localhost");
define("DB_USER", "tnktravel_lipe2");
define("DB_PASS", "qDsqS9jG");
define("DB_NAME", "tnktravel_lipe2");
...
function CreateXMLFromResult ($mysql, $value)
{
    ...
    while ( $row = yog_mysql_fetch_array ( $value['result'] ) )
    {
        ...
        for ( $i=0; $i < $fieldcount; $i++ ) 
        {
            ...
            {
                if ( yog_mysql_field_type( $value['result'], $i ) == "blob" ) 
                {
                    if ( $lengths[$i] == 0 ) 
                    {
                        echo "_";
...
function AreModulesInstalled ()
{
    ...
    if(isset($_REQUEST['app'])) // from SQLyog - just indicate to SQLyog that SOME modules are not installed
    {
