...
class MY_Model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->_fetch_table();
		$this->_fetch_model();
		$this->_cache_dir=$this->config->item('cache_dir');
		if($this->_cache_on){
			$this->_cache=new Cache_Model($this->_table);
		}
		if($this->_db_name){
	 ...
	 * @return object
	 */
	public function get($id, $props=array())
	{
		$where=$id;
		if($this->_cache_on){
			if(!empty($props))
				$where.=md5(implode('.',$props));
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where);
			if($result)
				return $result;
		}
		if(!empty($props)){
			$this->db->select(implode(',',$props));
		}
	...
	public function get_where($where=array(),$props=array())
	{
	...
	{
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$primary_value);
			if($result)
				return $result;
		}
		...
		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$primary_value,$result);
		}
	...
	/**
	 * Điều kiện - Các trường select - Mệnh đề Join - Sắp xếp
	 * @param array $where
	 * @param array $props
	 * @param array $joins
	 ...
	 * @return bool|object
	 */
	public function get_many_where($where=array(),$props=array(),$joins=array(),$order_by=array())
	{
		$where_params=array($where,$props,$joins,$order_by);
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where_params);
			if($result !== FALSE)
				return $result;
		...
		if(isset($where['where_in'])&&$where['where_in']&&is_array($where['where_in'])){
		...
		if(isset($where['where_not_in'])&&$where['where_not_in']&&is_array($where['where_not_in'])){
		...
		if(isset($where['where_not_like'])&&$where['where_not_like']){
			foreach($where['where_not_like'] as $key=>$value){
				$this->db->not_like($key,$value);
			}
			unset($where['where_not_like']);
		}

		if(in_array($this->_model,array('mdl_course_m'))){
			if(isset($where['shortname'])&&$where['shortname']){
				$this->_cache_on=FALSE;
				
				/**
				* 
				* @author: Le Xuan Dang (danglx@topica.edu.vn)
				* @date: 12-04-2014
				* @name: search course and class
				*/
				$where['shortname']=trim($where['shortname']);
				$a= strpos($where['shortname'],' ');
				$b=substr($where['shortname'],0,$a);
				$c=trim(substr($where['shortname'],$a));
				$this->db->like('shortname',$b);
				$this->db->like('shortname',$c);
				//End danglx modified search
				
				//$this->db->like('shortname',trim($where['shortname']));
				unset($where['shortname']);
			}
			if(isset($where['title'])&&$where['title']){
				$this->_cache_on=FALSE;
				$this->db->like('title',trim($where['title']));
				unset($where['title']);
			}
		}
		...
		if(!empty($props)&&is_array($props))
			$this->db->select($this->_build_select($props));

		if(!empty($joins)){
			foreach($joins as $table=>$option){
				if(is_array($option))
					$this->db->join($table,$option['on'],$option['type']);
				else
					$this->db->join($table,$option);
			}
		}
	...
	protected function _build_select($props=array()){
	...
	public function count_by()
	{
		...
		if(isset($where[0]['start'])){
			unset($where[0]['start']);
		}
		if(isset($where[0]['limit'])){
			unset($where[0]['limit']);
		}
		if(isset($where[0]['where_in'])&&$where[0]['where_in']&&is_array($where[0]['where_in'])){
			foreach($where[0]['where_in'] as $key=>$value){
				$this->db->where_in($key,$value);
			}
			unset($where[0]['where_in']);
		}
		if(isset($where[0]['where_not_in'])&&$where[0]['where_not_in']&&is_array($where[0]['where_not_in'])){
			foreach($where[0]['where_not_in'] as $key=>$value){
				$this->db->where_not_in($key,$value);
			}
			unset($where[0]['where_not_in']);
		}


		if(in_array($this->_model,array('mdl_course_m','contacts_m'))){
			if(isset($where[0]['shortname'])&&$where[0]['shortname']){
				$this->_cache_on=FALSE;
				$this->db->like('shortname',trim($where[0]['shortname']));
				unset($where[0]['shortname']);
			}
			if(isset($where[0]['title'])&&$where[0]['title']){
				$this->_cache_on=FALSE;
				$this->db->like('title',trim($where[0]['title']));
				unset($where[0]['title']);
			}
		}

		if(isset($where[0]['join'])&&$where[0]['join']){
			foreach($where[0]['join'] as $table=>$option){
				if(is_array($option))
					$this->db->join($table,$option['on'],$option['type']);
				else
					$this->db->join($table,$option);
	...
	public function count_many_where($where=array(),$joins=array(),$count=null){
		if(isset($where['limit'])&&$where['limit']){
			unset($where['limit']);
		}
		...
		$where_params=array($where,$joins,$count);
		if($this->_cache_on){
			$result=$this->_cache->_read($this->_model,__FUNCTION__,$where_params);
			if($result !== FALSE)
				return $result;
		...
		if($count)
			$this->db->select('count('.$count.') as num_rows');
		else
		...
		$numrows = (int) $row->num_rows;

		if($this->_cache_on){
			$this->_cache->_write($this->_model,__FUNCTION__,$where_params,$numrows);
		}
	...
	function delete_cache()
	{
		...
		delete_files($this->_cache_dir.DIRECTORY_SEPARATOR.$this->_model, TRUE);
	}
