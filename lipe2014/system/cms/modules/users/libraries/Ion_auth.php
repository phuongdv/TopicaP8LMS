...
class Ion_auth
{
	...
	public function forgotten_password($identity)    //changed $email to $identity
	{
		if ( $this->ci->ion_auth_model->forgotten_password($identity) )   //changed
		{
			...
			if(!call_send_mail($data)){
				$this->set_error('Gửi mail bị lỗi');
				return false;
			}
	...
	public function forgotten_password_complete($code)
	{
		...
		if ($new_password)
		{
			...
			//$results = Events::trigger('email', $data, 'array');
			if(!call_send_mail($data)){
				$this->set_error('password_change_unsuccessful,send mail error');
				return false;
			}
	...
	public function activation_email($id = 0)
	{
		...
		if(!call_send_mail($data)){
			$this->set_error('activation_email_unsuccessful,send mail error');
			return false;
		}
