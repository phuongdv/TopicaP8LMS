...
class Offline extends Public_Controller
{
	...
	function course($course_id=0){
		...
		$arr_lop=array(''=>'==Search by Classes==');
		...
		//anh hoá cho offline
		$MHV = $this->lang->line ('Mã học viên');
		$THV = $this->lang->line ('Tên học viên');
		$HTD  = $this->lang->line ('Họ và tên đêm');
		$LOP = $this->lang->line ('Lớp');
		$NHOM = $this->lang->line ('Nhóm');
		$SBO = $this->lang->line ('Số buổi offline');
		$DIEM = $this->lang->line ('Điểm');
		$CN = $this->lang->line ('Cập nhật');
		

		$this->template
			->title('Offline grades management : '.$course->shortname)
			->set('title','Offline grades management : '.$course->shortname)
			->set('course',$course)
			->set('rows',$rows)
			->set('arr_lop',$arr_lop)
