...
_wiris_plugin_url = '<?php echo $CFG->wwwroot?>/pluginwiris/editor/';
_wiris_filter_url = '<?php echo $CFG->wwwroot?>/pluginwiris/filter/';
...
Wiris.I18N = {
<?php if($CFG->wirisformulaeditorenabled) { ?>
  "WRS-wiris-formula" : "Wiris Editor"
 <?php if($CFG->wiriscasenabled) { ?>
  ,
 <?php } ?>
<?php } ?>
<?php if($CFG->wiriscasenabled) { ?>
...
Wiris.btnList = [
<?php if($CFG->wirisformulaeditorenabled||$CFG->wiriscasenabled) { ?>
	null, // separator
<?php } ?>
<?php if($CFG->wirisformulaeditorenabled) { ?>
	["wiris-formula"],
...
Wiris.doInsertion = function(result,type,editor,sel,parent){
	...
	if(result){
		...
		if(parent) {
			...
			// if we are in IE, the selection type might be control and the insertion won't work. 
			// To avoid this, we clear the selection contents (we will replace them)    
			if(HTMLArea.is_ie){
			...
			<?php if($CFG->wirisformulaeditorenabled) { ?>
				case "formula":
					...
					var m;
					if(source){m=imgFromSrc (source[0]);}
					else{m = imgFromSrc (imgCode);}
...
//at the same time
	...
	do {
		applet = subMathml(html, ReAppletIni, ReAppletFi, lastending);		// Getting applet buffer information
		formula = subMathml(html, ReMathIni, ReMathFi, lastending);			// Getting formula buffer information
...
function subMathml(html, ReIni, ReFi, pos) {
	...
	if (opened) {
		...
		if (closed) {
			return { start: opened.index, end: closed.index + closed[0].length };
		}
	}
	
	return { start: html.length, end: 0 };
}
...
function close_fullscreen(aEvent) {
	...
	{//We shouldn't enter here, added for safety purpose
		update_parent();
