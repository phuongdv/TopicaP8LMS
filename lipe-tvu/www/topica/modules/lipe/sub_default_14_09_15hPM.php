...
function default_report(){
	...
	$arrCourse = $clsCourse->GetAll();
	$sqlUser = "select * from mdl_user where id in (select distinct userid from mdl_course_display where course = $c_id) and id not in (select userid from mdl_role_assignments where roleid in (1,2,3,4,8) )";
...
$arrUser = $dbconn->GetAll($sqlUser);
	...
	$sqlForumPosts = "select count(*)  from mdl_forum_posts where created BETWEEN UNIX_TIMESTAMP('$st') and UNIX_TIMESTAMP('$en') and discussion in (select id from mdl_forum_discussions where forum = $fo)";
