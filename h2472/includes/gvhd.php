...
/*$log->logThis($log->get_formatted_date());
$log->logThis($profiles['username']);
$log->logThis("Vào khu vực GVHD");*/


 $template -> set_filenames(array(

	'gvhd'	=> $dir_template . 'gvhd.tpl')
...
$template -> assign_vars(array(
	...
	getCourseForGVHD($profiles['id'],$sname);

	// --------------------lấy ra Giang vien , cvht  user có thể gán ----------------------------------------------*/

	getGVCVHTForGVHD($sname,$profiles['id'],$gv);
...
WHERE
u.id={$profiles['id']}
and (r.id=14 or r.id=4))";

	if(!isset($sname)||$sname==''||$sname==0)
	...
	$sql_count_reply_answer="select * from tblanswer where $sql_po_course  and (status = 3 or status=0)";
	...
	$sql_qus_reply="select count(*) from tblreply where answerid in(select id from tblanswer where thread in (select id from tblthread where $sql_po_course $sql_po_course_filter $sql_po_assigner $sql_po_topic $sql_status $sql_po_delay $sql_searchtext $sql_attach))";
	...
	$sql="select count(*) from tblthread where $sql_po_course $sql_po_course_filter $sql_po_assigner $sql_po_topic $sql_status $sql_po_delay $sql_searchtext $sql_attach and assignid='".$profiles['id']."'  and (status = 0 or status = 3)";

	$result = $db->sql_query($sql);

	$count=$db->sql_fetchrow($result);

	$thr_norep=$count['count(*)']; 
	...
	// số câu hỏi của cá nhân cần trả lời

	

	$sql="select count(*) from tblthread where assignid='".$profiles['id']."' and status=3";
   ...
   $template -> assign_block_vars('L_ANWSER', array(
	...
	$template -> assign_vars(array(
  	 ...
  	 $template->assign_vars(array(
			...
			'question'       =>($unrep_thread==0) ? '':''.$unrep_thread.'</a>' ));	
