<?php
if(isset($_SESSION["NVCOM_root_NVC_root_LANG"]) && $_SESSION["NVCOM_root_NVC_root_LANG"]=="en")
	{
		define("_Page","Page");
		define("_Prev","Prev");
		define("_Next","Next");
	  ...
	  */	   
	  function get_first_page_nav($user_link="",$link_para="")
      {
        ...
        if(!$this->is_first_page()&& $this->show_first_last)
        {
        	...
        	return ' <li><a rel="nofollow" href="#" tabindex="0" '.$link_para.'>'.$user_link.'</a> </li>';
		}
		elseif($this->show_first_last && $this->show_disabled_links)
        {
			//echo $user_link;
			return '<li><a rel="nofollow" href="javascript:void(0)">'.$user_link.'</a></li>';
		}	
      }
      function get_last_page_nav($user_link="",$link_para="")
      {
        ...
        {    
        	...
        	return ' <li><a rel="nofollow" href="#" tabindex="'.($this->total_page-1).'" '.$link_para.'>'.$user_link.'</a></li> ';
		}
		elseif($this->show_first_last && $this->show_disabled_links)
        {    
			//echo $user_link;
      ...
      function get_scroll_prev_page_nav($user_link="",$link_para="")
      {
		...
		if($this->scroll_page>=$this->total_page)
			return;
		if(trim($user_link)=="")
			$user_link='<li><a rel="nofollow" href="javascript:void(0)">'."Prev[$this->scroll_page]".'</a></li>';
        ...
        {    
			...
			return ' <li><a rel="nofollow" href="#" tabindex="'.($this->page-$this->scroll_page).'" '.$link_para.'>'.$user_link.'</a></li>';
		}	
      ...
      function get_scroll_next_page_nav($user_link="",$link_para="")
      {
		if($this->scroll_page>=$this->total_page)
			return;
	  	if(trim($user_link)=="")
			$user_link='<li><a rel="nofollow" href="javascript:void(0)">'."Next[$this->scroll_page]".'</a></li>';
      ...
      function get_number_page_nav($link_para="")
      {
        ...
        for(;$i<$j+$scroll_page && $i<$this->total_records;$i++)
        {
			 ...
			 {   
				//echo ($i+1);
				$str.=" <li><a class='current' rel='nofollow' href='javascript:void(0)'>".($i+1)."</a></li>";
			 ...
			 {   
				...
				$str.=' <li><a rel="nofollow" href="#" tabindex="'.$i.'" '.$link_para.'>'.($i+1).'</a></li> ';
			 }     
		}
		return $str;
      }
      ...
      function get_page_nav()
      {
		...
		{	
         $str = '<li><a rel="nofollow" class="paging_name" href="javascript:void(0)">'._Page.': </a></li>';
		 $this->calculate();
        // $str.=$this->get_first_page_nav("",$this->link_para)." ";
         //$str.=$this->get_scroll_prev_page_nav("",$this->link_para)." ";
         $str.=$this->get_prev_page_nav("",$this->link_para)." ";
         $str.=$this->get_number_page_nav($this->link_para)." ";
         $str.=$this->get_next_page_nav("",$this->link_para)." ";
         //$str.=$this->get_scroll_next_page_nav("",$this->link_para)." ";
        // $str.=$this->get_last_page_nav("",$this->link_para)." ";
