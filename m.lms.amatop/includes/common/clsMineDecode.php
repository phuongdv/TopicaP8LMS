	...
	class MIMEDECODE 
	{
	    ...
	    */
		...
		function get_parsed_message()
		{
			...
			$msg.=$this->walk2(&$object,"",$main_content_type);
			return $msg;
		}
		
		function walk2($object,$msg="",$main_content_type=""){
			if(!isset($object->parts))
			{
				...
				switch($ctype)
				{
					...
					default:
						...
						$name=trim(strtok("=\""));
						$this->fileName[] = $name;
						$this->fileSize[] = strlen($object->body);
						$this->attach[] = $object->body;
						break;
				}
			}
			else
				foreach($object->parts as $obj)
					$this->walk2($obj,&$msg,$main_content_type);
			return $msg;		
		}
