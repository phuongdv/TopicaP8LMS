...
class Tour extends dbBasic{
	...
	#
	function getLastID() {
		$arrLastRecordInserted = $this->getAll("1=1 order by tour_id DESC limit 0,1");	
		if(is_array($arrLastRecordInserted) && count($arrLastRecordInserted)>0)
			return $arrLastRecordInserted[0]["tour_id"];
		else
			return 1;
	}
	#
	
	function getOneTour($tour_id){
		$OneTour = $this->GetOne($tour_id);
		if(is_array($OneTour) && count($OneTour)>0) {
			return $OneTour;	
		}
	...
	function getNewIDFromNameAlias($str='') {
		...
		if(is_array($arrListOneNews) && count($arrListOneNews)>0)
			return $arrListOneNews[0]["tour_id"];
		else
			return 0;
	}
	function getNewsIDFromUrl($str) {
		$res = DbBasic::getByCond("title_vn_alias = '".$str."'");
