...
class Setting_Calendar extends DbBasic{
	...
	function getTuanTong($c_id){
		global $dbconn;
		
		$sql = "select id, week_name, start_date, end_date from huy_setting_calendar where c_id='".$c_id."' and (week_name like '%Tổng%') limit 0,1";
 ...
 UNIX_TIMESTAMP('$start') and UNIX_TIMESTAMP('$end')";
	*/
	...
	function countPostInWeekVBB($username,$start,$end,$active_id) {
		/*-------------------------------Chu y khi cai dat server/thay doi CSDL----------------------------------------*/
	...
	// lay ra diem btvn so x
	
	function getbtvn($u_id,$c_id,$_btvn) {
		global $dbconn;
		...
		//$res = $dbconn->GetOne($sq4);
		$sql = "select qg.* from 
		       mdl_quiz_grades qg INNER join mdl_quiz mq on qg.quiz = mq.id 
		       where mq.course = $c_id and qg.userid = $u_id and mq.name = 'Bài tập về nhà số $_btvn'";
	...
	function get_grade_btvn_from_id($u_id,$c_id,$id_btvn) {
		global $dbconn;
		$sql = "select qg.* from 
		       mdl_quiz_grades qg INNER join mdl_quiz mq on qg.quiz = mq.id 
		       where mq.course = $c_id and qg.userid = $u_id and mq.id = $id_btvn";
		$res = $dbconn->getAll($sql);
	    return round($res[0]['grade']);
	}
	
	function get_grade_btn_from_id($u_id,$c_id,$id_btn) {
		global $dbconn;
		$sql = "select qg.* from 
		       mdl_quiz_grades qg INNER join mdl_quiz mq on qg.quiz = mq.id 
		       where mq.course = $c_id and qg.userid = $u_id and mq.id = $id_btvn";
	...
	function getgeadescale($c_id,$_btvn) {
		global $dbconn;	
		//$sql4 = "select grade from mdl_quiz_grades where quiz = $active_id and userid = $u_id";
		//die($sql4);
		//$res = $dbconn->GetOne($sq4);
		$sql = "select * from 
		       mdl_quiz mq 
		       where mq.course = $c_id and mq.name = 'Bài tập về nhà số $_btvn'";
	      
		$res = $dbconn->getAll($sql);
		//$clsQuizGrades = new QuizGrades();		
		//$res = $clsQuizGrades->getAll("quiz = $active_id and userid = $u_id");
		//print_r($res);die();
	    return $res[0]['gradescale'];
		//  return $active_id;
		...
		//return $result_grades;
         ...
         function get_bt_grade ($uid,$cid)
        {
         global $dbconn;
         $sql = "select hsl.active_id from huy_setting_lipe hsl INNER join huy_setting_calendar hsc on hsc.id = hsl.calendar_id
where hsl.c_id = $cid and hsl.lipe_type='E' ORDER BY hsl.active_id asc";
         $res_quiz = $dbconn->GetAll($sql);
         //print_r($res_quiz);
         if(count($res_quiz)>0)
         {
             ...
             $i=1;
             $result='(';
             foreach ($res_quiz as $quiz)
             {
                  $result.= 'BTVN '.$i.$quiz['hsl.active_id'].': ' .$this->Grades($uid,$quiz['active_id']).'; ';
                  $i++;
                }
        ...
        function Grades($u_id,$active_id) {
...
";
    ...
    function getassignment($id,$uid)
	{
		
	global $dbconn;
	 $sql = "select grade from mdl_assignment_submissions where assignment = $id and userid =  $uid limit 0,1";
	 

	$res = $dbconn->GetAll($sql);
	 if($res[0]['grade']=='' or $res[0]['grade']=='-1')
	 return 0;
	 else
     return $res[0]['grade'];
	    ...
	    function getbtnhom($id,$uid)
	{
	global $dbconn;
	// lay nhom cua hv
	$sql = "select topica_nhom nhom from mdl_user where id = $uid";
	$res = $dbconn->GetAll($sql);
	$nhom =  $res[0]['nhom'];
	// lay diem theo nhom
	 $sql = "
			SELECT
				max(mas.grade) grade
			FROM
				mdl_assignment_submissions mas
			INNER JOIN mdl_user u ON u.id = mas.userid
			WHERE
				mas.assignment = $id
			and u.topica_nhom='$nhom'
