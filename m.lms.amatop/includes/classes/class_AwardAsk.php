...
*  @version		: 2.1.1
*/
class AwardAsk extends dbBasic{

	function AwardAsk() {
		$this->pkey = "award_ask_id";
		$this->tbl = "award_ask";
	}
	
	function getAllAwardAskInWeek() {
		$last_time_ofweek = $this->getLastTimeOfCurrentWeek();
		$first_time_ofweek = $this->getFirstTimeOfCurrentWeek();
		$arrListAllInCurrentWeek = $this->getAll("is_online=1 and start_date>=".$first_time_ofweek." and end_date<=".$last_time_ofweek." order by order_no ASC");
		
		if(is_array($arrListAllInCurrentWeek) && count($arrListAllInCurrentWeek)>0)
			return $arrListAllInCurrentWeek;
		else
			return "";
	}
	
	function getAllLastOtherAwardAsk() {
		$first_time_ofweek = $this->getFirstTimeOfCurrentWeek();
		$arrListAllAwardAnnounceInWeek = $this->getAll("is_online=1 and start_date<".$first_time_ofweek." order by award_ask_id ASC");
