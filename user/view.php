...
//SCM	$name =	$user->username;
//SCM	$user_b64 = base64_encode($name);

//SCM	echo "<form method=\"post\" name=\"hieu_f\" id=\"hieu_f\" action=\"http://210.245.87.137/SCMSWebPortal/StudentProfiles.aspx?tab=lylich&idhv=$user_b64&idcvht=\"><div><input type=\"hidden\" value=\"1\" name=\"id\"/><input type=\"submit\" value=\"SCM\"/></div></form>";
        ...
        echo "</div>";
        echo "</form>";
//SCM   echo '<input type="button" onclick="return openpopup(\'/user/view_in.php\', \'\',\'menubar=0,location=0,scrollbars,status,resizable,width=800,height=700\',0)" value="Xem TT" >';
    }
    ...
    if ($course->id != SITEID && empty($course->metacourse)) {   // Mostly only useful at course level
        ...
        if ($canunenrol) {
            ...
            echo '<input disable="disable" type="submit" value="'.s(get_string('unenrolme', '', $course->shortname)).'" />';
            echo '</div>';
            echo '</form>';
        }
    }
