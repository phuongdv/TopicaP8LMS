...
//  Lists all the users within a given course
	...
	//echo CONTEXT_COURSE;	  echo ' ';	 
	//echo SITEID;
  //  print_r($frontpagectx);  //  print_r($context);
...
/// Print my course menus
    ...
    if ($context->id != $frontpagectx->id) {       // echo $context->id.'&'.$frontpagectx->id;
    ...
    if (!empty($search)) {
        ...
        $wheresearch .= ' AND ('. $fullname .' '. $LIKE .'\'%'. $search .'%\' OR email '. $LIKE .'\'%'. $search .'%\' OR topica_lop '. $LIKE .'\'%'. $search .'%\' OR idnumber '.$LIKE.' \'%'.$search.'%\') ';
