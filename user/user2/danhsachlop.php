...
//  Lists all the users within a given course
    ...
    add_to_log($course->id, 'user', 'view all', 'danhsachlop.php?id='.$course->id, '');
...
/// and if so, set $currentgroup to reflect the current group
    ...
    $baseurl = $CFG->wwwroot.'/user/danhsachlop.php?contextid='.$context->id.'&amp;roleid='.$roleid.'&amp;id='.$course->id.'&amp;perpage='.$perpage.'&amp;accesssince='.$accesssince.'&amp;search='.s($search);
...
//  bo tab o tren cung   require_once($CFG->dirroot .'/user/tabs.php');
...
/// Print my course menus
    if ($mycourses = get_my_courses($USER->id)) {
        ...
        popup_form($CFG->wwwroot.'/user/danhsachlop.php?roleid='.$roleid.'&amp;sifirst=&amp;silast=&amp;id=',
...
//link scm - hieupd
    if (!isset($hiddenfields['scm'])) {
        $tablecolumns[] = 'scm';
        $tableheaders[] = get_string('hieuscm');
    }
...
//trang thai hoc vien - hieupd
    if (!isset($hiddenfields['status'])) {
        $tablecolumns[] = 'status';
        $tableheaders[] = get_string('topica_status');
    }
    ...
    if (count($rolenames) > 1) {
        ...
        popup_form("$CFG->wwwroot/user/danhsachlop.php?contextid=$context->id&amp;sifirst=&amp;silast=&amp;roleid=", $rolenames,
                   'rolesform', $roleid, '');
        echo '</div>';
...
//base64 decode username cho link scm - hieupd
$name =	$user->username;
$ba64us = base64_encode($name);

//link scm - hieupd
                if (!isset($hiddenfields['scm'])) {
                    $data[] = '<strong><a href="http://210.245.9.197/scm/viewProfiles.aspx?id='.$ba64us.'">'.$user->username.'</a></strong>';
                }
...
//trang thai hoc vien - hieupd
//SELECT topica_status FROM `mdl_user` where id = $user->id


                if (!isset($hiddenfields['status'])) {
                    $data[] = '<strong>'.$user->username.'</strong>';
                }
    ...
    if ($bulkoperations && $totalcount > ($perpage*3)) {
        echo '<form action="danhsachlop.php" class="searchform"><div><input type="hidden" name="id" value="'.$course->id.'" />'.get_string('search').':&nbsp;'."\n";
