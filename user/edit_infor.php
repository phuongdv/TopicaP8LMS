<?php // $Id: edit.php,v 1.167.2.13 2008/12/08 20:15:39 skodak Exp $

    require_once('../config.php');
    require_once($CFG->libdir.'/gdlib.php');
    require_once($CFG->dirroot.'/user/edit_infor_form.php');
    ...
    //create form
    $userform = new user_edit_infor_form();
    ...
    if ($usernew = $userform->get_data()) {
        ...
        if ($CFG->emailchangeconfirmation) {
            ...
            if (isset($usernew->email) and $user->email != $usernew->email && !has_capability('moodle/user:update', $systemcontext)) {
                ...
                $email_changed_html .= print_continue("$CFG->wwwroot/course/", true);
                $email_changed = true;
            }
        }
        ...
        if (!$email_changed || !$CFG->emailchangeconfirmation) {
            //redirect("$CFG->wwwroot/user/danhsachlop.php?limit=30");
