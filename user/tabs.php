...
//        print_heading(format_string($site->fullname));
    ...
    /// Xem thong tin SCM - added by hieupd - 20100622


		$toprow[] = new tabobject('hieuscm', $wwwroot.'/user/scm.php', get_string('hieuscm'));
    ...
    /// Begin : Add info - thongtinthem
    if (!empty($showroles) and !empty($user)) { // this variable controls whether this roles is showed, or not, so only user/view page should set this flag
        $usercontext = get_context_instance(CONTEXT_USER, $user->id);
        if (has_capability('moodle/role:assign',$usercontext)) {
            $toprow[] = new tabobject('thongtinthem', $wwwroot.'/user/edit_infor.php?id='.$user->id.'&amp;course='.$course->id, get_string('thongtinthem'));

        }
    }

	/// End : Add info - thongtinthem
